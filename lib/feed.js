...
Feed.prototype.head = function (block, opts, cb) {
  ...
  function loop (err, node) {
    ...
    var nkey = self._prefix + roots.shift()
    var key = '!nodes!' + nkey
    if (knodes.hasOwnProperty(key)) loop(null, knodes[key])
    else self._core._nodes.get(nkey, loop)
  }
}
...
Feed.prototype.put = function (block, data, proof, cb) {
  ...
  function loop (err, node) {
    ...
    if (node && node.index === top.index) {
      ...
      batch.finish(function (err, docs) {
        if (err) cb(err)
        else self._db.batch(docs, writeStorage)
      })
      return
    }
  ...
  function index () {
    ...
    function loop (err, node) {
      ...
      if (!indexes.length) {
        ...
        else setTimeout(openStorage, 5) // There is a race condition in the protocol parser that requres this to be async. Investigate!
      } else {
...
Feed.prototype._append = function (buffers, cb) {
  ...
  batch.finish(function (err, docs) {
    if (err) cb(err)
    else self._db.batch(docs, self._afterAppend)
    if (self.storage) self.storage.append(self.blocks, buffers, self._afterAppend)
  })
}
...
Feed.prototype.getBlockFromHash = function (hash, cb) {
  var key = '!heads!' + this._prefix + hash.toString('hex')
  this._db.get(key, function (err, block) {
    if (err) cb(err)
    else cb(null, Number(block))
  })
}

Feed.prototype._nodeBatch = function () {
  var self = this
  var nodes = {}
  var mnodes = {}
  var batch = []
  return {
    add: function (node) {
      var key = '!nodes!' + self._prefix + node.index
      nodes[key] = node
      mnodes[key] = messages.Node.encode(node)
    },
    push: function (doc) {
      batch.push(doc)
    },
    finish: function (cb) {
      ...
      Object.keys(nodes).forEach(function (key) {
        pending++
        var node = nodes[key]
        self.head(node.index, { nodes: nodes }, function (err, hash) {
          ...
          if (hash) { // sometimes hash is undefined...
            batch.push({
              type: 'put',
              key: '!heads!' + self._prefix + hash.toString('hex'),
              value: String(node.index)
            })
          }
          if (--pending === 0) cb(null, batch)
        })
      })
      if (--pending === 0) cb(null, batch)
    }
